...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .querySelector("#bookingForm input[type='submit']")
  ...
  document
    ...
    .addEventListener("submit", function (e) {
      if (!validateForm()) {
        //manually call the function to validate input
        e.preventDefault();
        return;
      }
      ...
      fetch("bookEventFunctionality.php?id=" + eventID, {
        ...
        .then((data) => {
          ...
          if (data.message == "Varaus päivitetty!") {
            ...
            document.getElementById("backToEvents").innerText =
              "Kaikki tapahtumat"; //change the text of the button for event list
...
function selection(i) {
  ...
  if (bookedSeats.includes(Number(i))) {
  ...
  if (selectedSeats.includes(i) && eventType == 1) {
    ...
    if (exceedsLimit(1)) {
      if (!limitAlertShown) {
        alert(
          "On mahdollista varata enintään 2 paikkaa. Lisää paikkoja voi varata täyttämällä lomakkeen. Omat varaukset voi muokata oma tili -sivulla.",
        );
...
function validateForm() {
  ...
  if (selectedSeats.length === 0 && eventType == 1) {
    //check if at least one seat is selected. Only for event type 1!
...
document.getElementById("morePlaces").addEventListener("submit", function (e) {
  ...
  fetch("moreBookings.php", {
    // sends the data to php using POST
    method: "POST",
    body: formData,
  })
...
/* ========================================
   MSG for incorrect inputs
   ======================================== */

/*

if (UsersEnteredValue > seatsLeft) {
  document.getElementsByClassName("places-input") = "lala";
}
